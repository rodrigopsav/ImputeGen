...
# freqRef <- fread(paste0("ref_",pos, ".afreq"), header=T, select=c(5), sep = "\t")
# freqRef$pop <- "reference"
freqObs <- fread(paste0("obs_",pos, ".afreq"), header=T, select=c(5), sep = "\t")
freqObs$pop <- "validation"
freqPred <- fread(paste0("pred_",pos, ".afreq"), header=T, select=c(5), sep = "\t")
freqPred$pop <- "imputation"

### Concatenate files
freq <- rbind(freqObs, freqPred)
...
ldObs <- fread(paste0("obs_",pos, ".LDkit"), header=F, skip = 1, select=c(2,3), sep = "\t")
ldObs$pop <- "validation"
ldPred <- fread(paste0("pred_",pos, ".LDkit"), header=F, skip = 1, select=c(2,3), sep = "\t")
ldPred$pop <- "imputation"

### Concatenate files
ld <- rbind(ldObs, ldPred)
ld <- as.data.frame(ld)
...
# ldRef$pop <- "reference"
# ldObs <- fread(paste0("obs_",pos, ".ld"), header=T, select=c(2,5,7), sep = " ")
# ldObs$pop <- "validation"
# ldPred <- fread(paste0("pred_",pos, ".ld"), header=T, select=c(2,5,7), sep = " ")
# ldPred$pop <- "imputation"
# 
# ### Concatenate files
# ld <- rbind(ldRef, ldObs, ldPred)
# ld <- as.data.frame(ld)
# 
...
# ldList <- list()
# population <- c("reference", "validation", "imputation")
# 
# for (i in 1:3){
...
g1 <- arrangeGrob(grobs=lapply(plots_page1,"+", margin), nrow=2, 
                  layout_matrix = matrix(c(1,2,
                                           3,3), nrow=2, byrow=T))
ggsave(file=paste0("page5_", pos, ".pdf"), 
