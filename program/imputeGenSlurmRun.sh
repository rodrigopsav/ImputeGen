...
mkdir -p $OUTPUT_DIR/imputeGen_"$OUTPUT_NAME"/logSlurm 2>&1 > /dev/null
mkdir -p $OUTPUT_DIR/imputeGen_"$OUTPUT_NAME"/params 2>&1 > /dev/null
rm $OUTPUT_DIR/imputeGen_"$OUTPUT_NAME"/logSlurm/jobid1.txt > /dev/null 2>&1
export ANALYSIS_ID=$(echo $RANDOM)


### Run ImputeGen
cp $PARAMETERS $OUTPUT_DIR/imputeGen_"$OUTPUT_NAME"/params/"$ANALYSIS_ID"_"$(basename $PARAMETERS)"
cp $CONFIGSLURM $OUTPUT_DIR/imputeGen_"$OUTPUT_NAME"/params/"$ANALYSIS_ID"_"$(basename $CONFIGSLURM)"
source $IMPUTEGEN_DIR/program/slurm/runSubmitJobsSlurm.sh
echo "Running ImputeGen"
echo "ANALYSIS NAME: $OUTPUT_NAME"
echo "To check all the jobs, run the command line: $IMPUTEGEN_DIR/imputeGenSlurmLog.sh -a $ANALYSIS_ID -o $OUTPUT_DIR/imputeGen_${OUTPUT_NAME}"
echo
echo "Check the log files per chromosome in $OUTPUT_DIR/imputeGen_${OUTPUT_NAME}/log"
echo "To open the log file of a chromosome, type on terminal: tail -f $OUTPUT_DIR/imputeGen_${OUTPUT_NAME}/log/${OUTPUT_NAME}_CHR.txt"
echo "Just replace CHR by the chromosome name"
echo "To kill this ImputeGen analysis, run the command line:"
